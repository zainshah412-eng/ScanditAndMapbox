...
public class IdCaptureActivity2 extends CameraPermissionActivity
    ...
    Boolean isBackSideScanning = true;
...
//        dataCaptureManager.getIdCapture().setEnabled(false);
         ...
         * to the main thread.
         */
        isBackSideScanning = !isBackSideScanning;
        Vibrator vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);

// Vibrate for 1000 milliseconds (1 second)
        vibrator.vibrate(300);
        if (isBackSideScanning) {
            Log.wtf(
                    "PassportData",
                    "ID Capture -> ${Gson().toJson(getPassportDataAfterScanning(capturedId))}"
            );
